...
// Skips Google sign-in so the design can be tested without OAuth setup.
// REMOVE this block before promoting the preview to production.
const PREVIEW_SKIP_AUTH = true;
if (PREVIEW_SKIP_AUTH) {
  try {
    if (!localStorage.getItem('eco_user')) {
      localStorage.setItem('eco_user', JSON.stringify({
        name: 'Preview',
        email: 'preview@terra.ai',
        picture: ''
      }));
    }
  } catch (e) {}
